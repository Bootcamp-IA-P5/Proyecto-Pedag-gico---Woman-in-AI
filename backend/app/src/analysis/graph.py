...
ENABLE_LYRIC_SANITIZATION = (
    os.getenv("ANALYSIS_SANITIZE_LYRICS", "false").lower() == "true"
)
...
async def nodo_analizar(state: AnalysisState) -> dict:
    try:
        ...
        return {
            ...
            "puntuacion_global_preliminar": resultado.get(
                "puntuacion_global_preliminar", 0.0
            ),
...
async def nodo_consolidar(state: AnalysisState) -> dict:
    return {
        "resultado_final": {
            ...
            "puntuacion_global_preliminar": state.get(
                "puntuacion_global_preliminar", 0.0
            ),
