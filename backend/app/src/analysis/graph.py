...
MAX_LETRA_ANALISIS_CHARS = 12000
...
async def nodo_analizar(state: AnalysisState) -> dict:
    try:
        ...
        letra_para_analizar = letra.strip()
        letra_recortada = len(letra_para_analizar) > MAX_LETRA_ANALISIS_CHARS
        if letra_recortada:
            letra_para_analizar = letra_para_analizar[:MAX_LETRA_ANALISIS_CHARS]

        resultado = await reporter.analizar(letra_para_analizar)
        errores = resultado.get("errores") or []
        if letra_recortada:
            errores.append(
                {
                    "error": f"La letra se recortó a {MAX_LETRA_ANALISIS_CHARS} caracteres para evitar errores de tamaño en proveedores LLM.",
                }
            )
