        ...
        .insert(
            {
                ...
                **scores_para_modelo("groq"),
                "evidence_objectification": get_fragmentos(
                    "Objetificación Sexual", "groq"
                ),
                "evidence_roles": get_fragmentos("Sumisión / Roles de Género", "groq"),
                "evidence_degrading": get_fragmentos(
                    "Insultos / Lenguaje Degradante", "groq"
                ),
        ...
        .insert(
            {
                ...
                "model_name": os.getenv(
                    "OPEN_ROUTER_MODEL", "openrouter/gemini-2.5-flash"
                ),
                "prompt_version": resultado.get("prompt_version", "v1.0"),
                "temperature": 0.1,
                **scores_para_modelo("openrouter"),
                "evidence_objectification": get_fragmentos(
                    "Objetificación Sexual", "openrouter"
                ),
                "evidence_roles": get_fragmentos(
                    "Sumisión / Roles de Género", "openrouter"
                ),
                "evidence_degrading": get_fragmentos(
                    "Insultos / Lenguaje Degradante", "openrouter"
                ),
                "dominant_narrative": nivel_global,
                "gender_representation": gender_representation,
                "symbolic_roles": symbolic_roles,
...
async def analizar_letra_nueva(input: LyricInput):
    if len(input.letra.strip()) < 50:
        raise HTTPException(
            status_code=400, detail="La letra es demasiado corta para analizarla"
        )
...
async def analizar_por_id(song_id: int):
    ...
    # 4. Guardar (no interrumpir la respuesta si falla persistencia secundaria)
    try:
        ...
        errores = resultado.setdefault("errores", [])
        errores.append(
            {"error": f"No se pudo guardar la evaluación en Supabase: {exc}"}
        )
